...
export default function ProjectDetailPage() {
  ...
  const fetchProject = async () => {
    ...
    try {
      const res = await axios.get(`https://task-manager-02q1.onrender.com/projects/${projectId}`, {
  ...
  const fetchTasks = async () => {
    if (!projectId || !token) return;
    try {
      const res = await axios.get(`https://task-manager-02q1.onrender.com/projects/${projectId}/tasks`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setTasks(res.data);
    } catch (err) {
  ...
  const fetchLogs = async (taskId: string) => {
    try {
      const res = await axios.get(`https://task-manager-02q1.onrender.com/tasks/${taskId}/logs`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setLogs(res.data);
    } catch (err) {
  ...
  const handleDeleteTask = async (taskId: string) => {
    ...
    try {
      await axios.delete(`https://task-manager-02q1.onrender.com/tasks/${taskId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchTasks();
